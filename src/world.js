jump.World = (function () {
  ...
  World.prototype.setOptions = function (options) {
    ...
    var mean = Math.round(this.speed * 100000/this.limit);
    this.spawn = {
      counter: 0,
      limit: 0,
      mean: mean,
      bound: Math.round(mean*0.2),
    };
  };
  ...
  World.prototype.clickEvent = function (event) {
    this.clicked = this.renderer.mapToWorld(event);
  };
  ...
  World.prototype.score = function () {
    return Math.round(this.distanceTravelled * 100 + this.jumps*10);
  };
  ...
  World.prototype.clickEventProc = function (clicked) {
    if (clicked) {
      ...
      if (nearestAvailableCloud && nearestAvailableCloud.squaredDistanceTo(clicked) < 0.01) {
